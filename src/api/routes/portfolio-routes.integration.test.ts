...
beforeAll(async () => {
  ...
  try {
    ...
    /* seed may fail if DB schema changed */
  }
});
...
describe("PortfolioRoutes integration", () => {
  ...
  test("GET / fallback includes all seeded assets", async () => {
    ...
    const assetMap = new Map(
      body.assets.map((a: unknown) => [(a as Record<string, unknown>).asset, a])
    );
  ...
  test("GET /history returns snapshots", async () => {
    ...
    const res = await portfolioRoutes.request(`/history?from=${fromSecs}&to=${toSecs}`);
  ...
  test("GET /history snapshot includes holdings JSON", async () => {
    ...
    if (body.length > 0) {
      ...
      const holdings =
        typeof body[0].holdings === "string" ? JSON.parse(body[0].holdings) : body[0].holdings;
  ...
  test("GET /history snapshot includes allocations JSON", async () => {
    ...
    if (body.length > 0) {
      ...
      const allocations =
        typeof body[0].allocations === "string"
