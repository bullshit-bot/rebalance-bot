...
backtestRoutes.get("/backtest/list", async (c) => {
  try {
    const rows = await BacktestResultModel.find({}, { _id: 1, config: 1, metrics: 1, createdAt: 1 })
      .sort({ createdAt: -1 })
      .lean();
...
backtestRoutes.post("/backtest/optimize", async (c) => {
  ...
  try {
    const request: OptimizationRequest = {
      ...
      timeframe: (body as OptimizationRequest).timeframe ?? "1d",
      includeCashScenarios:
        typeof b["includeCashScenarios"] === "boolean" ? b["includeCashScenarios"] : false,
    };
