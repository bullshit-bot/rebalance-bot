...
class DriftDetector {
  ...
  start(): void {
    ...
    this.active = true
    eventBus.on('portfolio:update', this.portfolioListener)
  ...
  stop(): void {
    if (!this.active) return
    this.active = false
    eventBus.off('portfolio:update', this.portfolioListener)
  ...
  private handlePortfolioUpdate(portfolio: Portfolio): void {
    ...
    eventBus.emit('rebalance:trigger', { trigger: 'threshold' })
  }
}
