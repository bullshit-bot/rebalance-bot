...
class DriftDetector {
  ...
  private handlePortfolioUpdate(portfolio: Portfolio): void {
    ...
    if (gs?.trendFilterEnabled) {
      ...
      const bearCashPct =
        typeof gs.bearCashPct === "number" ? gs.bearCashPct : DEFAULT_BEAR_CASH_PCT;
      const cooldownDays =
        typeof gs.trendFilterCooldownDays === "number" ? gs.trendFilterCooldownDays : 3;
      ...
      if (!isBull) {
        ...
        const cashPct = portfolio.totalValueUsd > 0 ? (cashUsd / portfolio.totalValueUsd) * 100 : 0;

        if (cashPct < bearCashPct) {
          console.info(
            `[DriftDetector] Bear market — cash=${cashPct.toFixed(1)}% < target=${bearCashPct}%, triggering defensive rebalance`
          );
          this.lastRebalanceTime = Date.now();
    ...
    const threshold =
      gs?.dcaRebalanceEnabled && gs?.hardRebalanceThreshold
    ...
    console.info(
      `[DriftDetector] Threshold breached — asset=${breachedAsset.asset} drift=${breachedAsset.driftPct.toFixed(2)}% (threshold=${threshold}%)`
    );
