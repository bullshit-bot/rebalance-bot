...
import type { ExchangeCredentialsMap, ExchangeManagerDeps } from "./exchange-manager";

// ─── Helpers ─────────────────────────────────────────────────────────────────

/** Build a deterministic credentials map without relying on process.env */
function makeCredentials(
  exchanges: Partial<Record<ExchangeName, { apiKey: string; secret: string; password?: string }>>
): ExchangeCredentialsMap {
...
function makeDeps(options?: {
  ...
  const credentials =
    options?.credentials ??
    makeCredentials({ binance: { apiKey: "test-key", secret: "test-secret" } });
