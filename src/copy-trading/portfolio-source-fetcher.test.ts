...
describe("PortfolioSourceFetcher", () => {
  describe("fetch", () => {
    ...
    it("should reject HTTP URLs", async () => {
      await expect(
        portfolioSourceFetcher.fetch("http://example.com/portfolio.json")
      ).rejects.toThrow("Only HTTPS URLs are allowed");
    });

    it("should reject localhost", async () => {
      await expect(
        portfolioSourceFetcher.fetch("https://localhost:8080/portfolio.json")
      ).rejects.toThrow();
    });

    it("should reject 127.0.0.1", async () => {
      await expect(
        portfolioSourceFetcher.fetch("https://127.0.0.1:8080/portfolio.json")
      ).rejects.toThrow();
    });

    it("should reject private IP 10.x.x.x", async () => {
      await expect(
        portfolioSourceFetcher.fetch("https://10.0.0.1/portfolio.json")
      ).rejects.toThrow();
    });

    it("should reject private IP 192.168.x.x", async () => {
      await expect(
        portfolioSourceFetcher.fetch("https://192.168.1.1/portfolio.json")
      ).rejects.toThrow();
    });

    it("should reject private IP 172.16-31.x.x", async () => {
      await expect(
        portfolioSourceFetcher.fetch("https://172.16.0.1/portfolio.json")
      ).rejects.toThrow();
    });

    it("should reject 0.0.0.0", async () => {
      await expect(
        portfolioSourceFetcher.fetch("https://0.0.0.0/portfolio.json")
      ).rejects.toThrow();
    });
    ...
    it("should reject 172.17.x.x (private IP)", async () => {
      await expect(
        portfolioSourceFetcher.fetch("https://172.17.0.1/portfolio.json")
      ).rejects.toThrow();
    });

    it("should reject 172.31.x.x (private IP)", async () => {
      await expect(
        portfolioSourceFetcher.fetch("https://172.31.255.1/portfolio.json")
      ).rejects.toThrow();
    });

    it("should reject 0.x.x.x range", async () => {
      await expect(
        portfolioSourceFetcher.fetch("https://0.1.1.1/portfolio.json")
      ).rejects.toThrow();
    });
    ...
    it("should reject allocations not summing to ~100%", async () => {
      ...
      await expect(portfolioSourceFetcher.fetch("https://example.com/data.json")).rejects.toThrow(
        "expected ~100%"
      );
    });
    ...
    it("should reject missing asset field", async () => {
      globalThis.fetch = async () =>
        new Response(JSON.stringify([{ targetPct: 50 }]), { status: 200 });
    ...
    it("should reject invalid targetPct", async () => {
      globalThis.fetch = async () =>
        new Response(JSON.stringify([{ asset: "BTC", targetPct: "invalid" }]), { status: 200 });
    ...
    it("should reject negative targetPct", async () => {
      globalThis.fetch = async () =>
        new Response(JSON.stringify([{ asset: "BTC", targetPct: -10 }]), { status: 200 });
    ...
    it("should reject unexpected response shape", async () => {
      globalThis.fetch = async () => new Response(JSON.stringify({ foo: "bar" }), { status: 200 });
    ...
    it("should reject empty asset string", async () => {
      globalThis.fetch = async () =>
        new Response(JSON.stringify([{ asset: "   ", targetPct: 50 }]), { status: 200 });
